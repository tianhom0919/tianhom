...
|---------------------|-------------------------------------------------|--------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| **core-image-minimal** | A minimal file system for booting the system.  | - Contains only the essential components required to boot Linux.<br>- No graphical interface or advanced features. | - Testing basic hardware functionality.<br>- When minimal storage or RAM is available.<br>- Acts as a foundation for custom images. |
| **core-image-bsp**     | Board Support Package (BSP) image for specific hardware. | - Includes components required for the target board (e.g., drivers, kernel modules, utilities).<br>- Provides basic development tools. | - Validate board-specific features (e.g., peripherals, I/O).<br>- Starting point for adding application-specific software. |
| **core-image-weston**  | Graphical system image using the Weston compositor. | - Includes a lightweight Wayland compositor (Weston).<br>- Suitable for graphical interface development. | - Systems with displays requiring a GUI.<br>- Developing and testing graphical applications.  |
| **core-image-qt**      | An image with Qt framework support for GUIs.   | - Includes the Qt libraries and tools.<br>- Designed for running or developing applications with the Qt framework. | - Embedded systems with advanced graphical requirements.<br>- Devices needing touchscreen interfaces or modern UI elements. |
:::
